...
char* printNew2_Zbra_1(const string& printStr,int X, int Y,char* p)
{
	...
	for ( list<string>::iterator iter = printv.begin();iter != printv.end(); ++iter ){
		if( 0 == z ){ //二维码
			if (strstr(iter->c_str(),"http") != NULL)
			{
				p += sprintf(p,"^FT%d,%d^BQN,2,3^FDMA,%s^FS",X+164,Y+156,(char*)iter->c_str()); // 2代表二维码 3代表倍数
			}
			else if (iter->length()<5)
			{
				break;
			...
			{
				p += sprintf(p,"^FT%d,%d^BQN,2,4^FDMA,%s^FS",X+164,Y+176,(char*)iter->c_str()); // 2代表二维码 4代表倍数				
			}
...
char* printNew3_Zbra(const string& printStr,int X, int Y,char* p)
{
	...
	for ( list<string>::iterator iter = printv.begin();iter != printv.end(); ++iter ){
		if( 0 == z ){ //二维码	
			if (strstr(iter->c_str(),"http") != NULL)
			{
				p += sprintf(p,"^FT%d,%d^BQN,2,2^FDMA,%s^FS",X+130,Y+62,(char*)iter->c_str()); // 2代表二维码 1代表倍数
			}
			else if (iter->length()<5)
			{
				break;
			...
			{
				p += sprintf(p,"^FT%d,%d^BQN,2,4^FDMA,%s^FS",X+130,Y+76+20,(char*)iter->c_str()); // 2代表二维码 3代表倍数
			}
			y = Y + 84;
		}
		else if( 1 == z ){//数字码
			*iter=iter->substr((iter->find( "溯源码:")!=-1?iter->find("溯源码:")+7:0),iter->length());
			...
			if (iter->length()==0)
			{
				...
				p += sprintf(p,"^FT%d,%d^A2N,15,15^FH\^FD%s^FS",X+28,y,(char*)iter->c_str());
			y += 16;
		}
		z++;
	}
